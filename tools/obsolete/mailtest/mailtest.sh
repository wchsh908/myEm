...
prepare()
{
	echo "select emailaddress into outfile '/tmp/to.txt' from emarketer.tmp_list_subscribers where domainname = '$domainname' limit $total;" | mysql -u root -pwchsh908
	echo "delete from emarketer.tmp_list_subscribers where domainname = '$domainname' limit $total;" | mysql -u root -pwchsh908
	mv -f /tmp/to.txt $basedir

	recipients=($(cat $basedir/to.txt))
	count_recipients=${#recipients[*]}
	
	if [ -f $basedir/from.txt ]
	then
		senders=($(cat $basedir/from.txt))
		count_senders=${#senders[*]}
	else
		echo "Error.Can not find file of senders.Quit."
		exit 2
	fi
...
do_test()
{
	...
	echo "   Send $total mails in $time minutes:"
