...
class Email_API
{
	...
	*/
	function _SetupHeaders()
	{
		...
		$headers .= '<___FROM___ADDR___>' . $this->_newline;
	...
	*/
	function _Send_Email(&$rcpt_to, &$to, &$subject, &$body, &$headers)
	{
		...
		$this->DebugMemUsage('rcpt_to: ' . $rcpt_to . '; to: ' . $to . '; subject: ' . $subject . '; headers: ' . $newheaders);
		...
		if (!$this->TestMode) {
			if ($this->SMTPServer) {
				...
				return $this->_Send_SMTP($rcpt_to, $to, $subject, $body, $newheaders);
			}
		}
		...
		*/
		...
		if ($this->safe_mode || !$this->BounceAddress) {
			if (!$this->TestMode) {
				$mail_result = mail($to, $subject, $body, rtrim($newheaders));
			} 
			else {
				$mail_result = true;
			}
			...
			//默认情况下，smtp是本地主机，这时调用sendmail命令。最后一个参数是-f sender（也是退信人）
			if (!$this->TestMode) {
				$mail_result = mail($to, $subject, $body, rtrim($newheaders), $this->_sendmailparameters);
			} else {
				$mail_result = true;
			}
		}
	...
	*/
	...
	{
		if ($this->SMTPServer != Wchsh::$lastSMTPServer)
		{
			$this->_Close_Smtp_Connection();
			$this->_smtp_email_count = 0;
			Wchsh::$lastSMTPServer = $this->SMTPServer;
		}
		...
		//先与smtp服务器端建立起连接。这一步相当于telnet server 25
		...
		//用户验证
		//参考文档：http://hi.baidu.com/pdprf/item/b555f355e3c0c89009be170f
		if ($requireauth && $username) {
			//实际上，如果一台机器有两个IP，假设一个叫A一个叫B，我们的email marketer位于A上，而smtp服务器位于B上
			//实验证明这种情况下虽然B需要身份验证，但由于A和B在同一台机器上了，也可以不用经过验证
			//
			if (!$password) {
				$this->ErrorCode = 3;
