...
class Bounce_API extends Jobs_API
{
	...
	function GetBounceFrom($header=false)
	{
		...
		reset($header->from);
		// we cannot juse use header->fromaddress because this might contain a name or something.
		// so we reconstruct it ourselves.
		$from_header = current($header->from);

		// If the from header doesnt have a required property return false also
